...
void CGame::Update(void)
{
	...
	if (m_bMovieFlag == true)
	{
		...
		if (pCam != NULL)
		{
			...
			{
				pCam->SetRotation(rot + D3DXVECTOR3(0.0f, -0.01f, 0.0f));
			}
			...
			if (pInputKeyboard->GetPress(DIK_NUMPAD8) == true)
			{
				...
				pCam->SetPosR(pCam->GetPosR() + move);
			}
			...
			{
				move = D3DXVECTOR3(sinf(rot.y + (D3DX_PI * 0.5f)) * 4.0f, 0.0f, cosf(rot.y + (D3DX_PI * 0.5f))*4.0f);
				pCam->SetPosR(pCam->GetPosR() + move);
			}
...
void CGame::CameraSetting(int nNumPlayer)
{
	...
	if (pCameraManager != NULL)
	{
		...
		//動画作成用カメラ作成
		pCameraManager->CreateCamera("MOVIE_CAMERA",CCamera::TYPE_TOPVIEW,
			D3DXVECTOR3(0.0f, -100.0f, 0.0f), D3DXVECTOR3(-0.5f, 0.0f, 0.0f), CAMERA_LENGTH_TOPVIEW_PLAY);
		pCameraManager->SetCameraViewPort("MOVIE_CAMERA", 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
		;
